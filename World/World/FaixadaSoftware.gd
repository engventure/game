...
func _on_FaixadaSoftware_body_entered(body):
	var go_in = get_parent().software_walk_in
	if body.name == "Player" and go_in:
		interativo = true
		get_node("Label2").visible = true
	
func _on_FaixadaSoftware_body_exited(body):
	if body.name == "Player":
		interativo = false
...
func _input(event):
	if Input.is_key_pressed(KEY_E) and interativo:
		...
		get_tree().change_scene("res://ArenaSoftware/ArenaSoftware/ArenaSoftware.tscn")
